...
trend_summary = trend_summary[trend_summary['start_value'] >= 100].copy()

# ============================================================
# HITUNG GROWTH %
# ============================================================

trend_summary['Growth_Pct'] = (
    (
        trend_summary['end_value']
        - trend_summary['start_value']
    )
    / trend_summary['start_value']
) * 100
...
sorted_report = trend_summary.sort_values(
    by='Growth_Pct',
    ascending=False
)
...
# ============================================================
    ...
    # ============================================================
    # TAMBAHAN CUSTOM LEGEND WARNA
    # ============================================================
    
    from matplotlib.lines import Line2D

    # Legend khusus warna ranking rising star
    custom_rank_legend = []

    for kode, rank in rank_mapping.items():

        nama_produk = (
            plot_df[plot_df['kode_produk'] == kode]
            ['nama_produk']
            .iloc[0]
        )

        custom_rank_legend.append(
            Line2D(
                [0],
                [0],
                color=color_mapping[kode],
                lw=3,
                marker='o',
    ...
    # Legend khusus Top Sales
    top_sales_legend = []

    for idx, (_, row) in enumerate(top3_sales.iterrows()):
        ...
        top_sales_legend.append(
            Line2D(
                ...
                linestyle='--',
                marker='o',
                label=f"Top Sales - {row['nama_produk']}"
            )
        )

    # Gabungkan legend
    all_custom_legend = top_sales_legend + custom_rank_legend
    ...
    # jarak legend dari plot
        borderaxespad=0.
    )
    
    plt.subplots_adjust(right=0.78)
    plt.tight_layout()
    plt.savefig('rising_star_index.png', bbox_inches='tight', dpi=300) # Sesuaikan nama file
...
if not trend_summary.empty:
    ...
    rising_star_final = rising_star_final.sort_values(
    by='Growth_Pct',
    ascending=False
).reset_index(drop=True)
